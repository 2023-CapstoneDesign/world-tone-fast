...
async def s3_download(key: str, group_key: str):
    try:
        local_file_path = os.path.join("tmp", group_key+"/aws/"+key)  # 로컬 파일 경로 설정
...
async def download(group_key: str | None = None):
    ...
    contents = [await s3_download(obj_key, group_key) for obj_key in object_keys]
    logger.info("SUCCESS")
    return "SUCCESS"
...
async def upload(file_name: str, saved_key: str | None = None) :
    ...
    try:
        # S3에 업로드
        s3_object_key = "uploads/" + saved_key + ".wav"
